...
const int n = 1024 << 4;                // size of the cost/pay matrix
const int log2_n = 10 + 4;              // log2(n) needs to be entered manually

const int n_threads = 1;                // Number of threads used in small kernels grid size (typically grid size equal to n)
...
int main(int argc, char* argv[]) {
    ...
    std::cerr << "load time\t" << std::chrono::duration_cast<std::chrono::milliseconds>(end-begin).count() << "ms" << std::endl;
        
    begin = std::chrono::high_resolution_clock::now();
    cudaMemcpyToSymbol(slack, pay, sizeof(data)*nrow*ncol);
    Hungarian_Algorithm << < 1, 1 >> > ();
    cudaDeviceSynchronize();
    cudaMemcpyFromSymbol(h_column_of_star_at_row, column_of_star_at_row, nrow*sizeof(int));
    end = std::chrono::high_resolution_clock::now();
    std::cerr << "run time\t" << std::chrono::duration_cast<std::chrono::milliseconds>(end-begin).count() << "ms" << std::endl;
    ...
    std::cerr << "score\t" << total_pay << std::endl;
}